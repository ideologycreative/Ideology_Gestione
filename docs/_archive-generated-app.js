/* IDEOLOGY STUDIO — application logic
 *
 ...
 * in a new module.
 *
 * Transforms applied: boot(31L), CLOUD.load(51L), CLOUD.saveNow(96L), CLOUD.setStatus(1L), getMyAssignment(11L), DROPBOX.getToken(10L), DROPBOX.upload(69L), theme init(4L), applyTheme(7L), preview avatar(1L), status badge tooltip(1L), squared-pills(18), pillar chip (under frame)(1L), pillar swatch(1L), format chip (under frame)(1L), format swatch(1L), status-tokens(46), status badge gutter(1L), drop format chip in cell(2L), plus 67 identifier renames.
 */
...
/**
 * genId — genera un id univoco con un dato prefisso.
 *
 * FIX: prima ogni punto del file generava id per conto proprio con
 * schemi tipo `'a_'+Date.now()` — SOLO un timestamp in millisecondi,
 * zero casualità. Due elementi creati nello stesso millisecondo
 * (facilissimo con azioni rapide in sequenza, o operazioni bulk)
 * ottengono lo STESSO id — esattamente il bug trovato in produzione:
 * due account social di due clienti diversi con id identico, scoperto
 * mentre si migravano i clienti verso le nuove tabelle relazionali.
 * Alcuni punti avevano già imparato ad aggiungere Math.random(), ma
 * in modo incoerente, ognuno per conto proprio. Questo è l'unico
 * generatore condiviso da usare ovunque serva un nuovo id.
 */
...
/* ══════════════════════════════════════════
   IDEOLOGY CLOUD — Supabase sync via /api/project
   Le credenziali Supabase restano server-side.
   Browser only knows IDEOLOGY SESSION via cookie HttpOnly (JWT).
══════════════════════════════════════════ */
const CLOUD = {
  apiUrl: window.location.origin + '/api/project',
  apiKey: '', // auth via HttpOnly cookie nassa_session — no client-side key
  user: 'ideology_studio', // shared across all users — one studio, one dataset
  _saveTimer: null,
  _status: 'idle',
  _booting: false,
  ...
  snapshot() {
    ...
    function cleanFeeds(feedsObj) {
      ...
      Object.keys(feedsObj||{}).forEach(k => {
        ...
        if (items.length > MAX_FEED_ITEMS) {
          console.warn('[Ideology] Feed key', k, 'has', items.length, 'items — pruning to', MAX_FEED_ITEMS);
    ...
    return { version:'2.0', exportedAt: new Date().toISOString(),
      ...
      // auditData/pdmData/pdcData/stratBuildData: legacy, non più la fonte
      // di verità (vedi commento esteso vicino a "let auditData = {}" più
      // sotto) — restano comunque nel payload perché /api/project fa un
      // replace completo del blob condiviso, non un merge: toglierle da
      // qui le cancellerebbe per sempre dal blob al prossimo salvataggio,
      // per un beneficio puramente estetico. Non aggiungere nuova logica
      // che scrive o legge da queste quattro — usare clients[idx].audit/
      // .pdm/.pdc/.strategy.
      highlights, pedPlans, notesData, ideologyDocs, pilastri, formati, landing, stampa, mood, shooting, brief, brandKit, reportData, qbrData, auditData, pdmData, pdcData, stratBuildData, adsCampaigns, sbBozze, ugcInfluencer,
  ...
  apply(data) {
    ...
    clients = (data.clients || []).map(cl=>({
      ...
      accounts: (cl.accounts||[]).map(acc=>{
        const merged = { profileImg: '', bio: '', ...acc };
        // Alcuni account arrivano da percorsi diversi con campi diversi —
        // Content storicamente usa "name", Week Plan (pannello cliente,
        // sezione Account social) usa "handle". Senza questo fallback un
        // account creato da Week Plan, o uno con solo l'id (com'è successo
        // durante il recupero dati di questa sessione), mostrava la
        // stringa letterale "undefined" ovunque il nome viene letto —
        // decine di punti in tutto il file, sistemarli uno per uno era
        // fragile. Meglio garantirlo qui, una volta sola, alla fonte.
        merged.name = merged.name || merged.handle || '';
        return merged;
      })
    }));
    ...
    clients.forEach(c => { if(!c.accounts) c.accounts=[]; if(!c.id) c.id=genId('c_'); });
    feeds = {};
    ...
    // ── DIAGNOSTICA LOAD ─────────────────────────────────────────────────
    console.group('%c[Ideology] apply() — loading from DB', 'color:#60a5fa;font-weight:700');
    ...
    // auditData/pdmData/pdcData legacy — vedi commento esteso vicino a
    // "let auditData = {}" più sotto in questo file. Fonte di verità
    // vera: clients[idx].audit/.pdm/.pdc, scritta da Strategia.
    auditData  = data.auditData  || {};
    pdmData    = data.pdmData    || {};
    pdcData    = data.pdcData    || {};
    stratBuildData = data.stratBuildData || {};
    sbBozze    = data.sbBozze    || {};
    ideologyDocs  = data.ideologyDocs  || {};
...
/* ══════════════════════════════════════════
   ...
   Builds structured paths under /Ideology Studio/IdeologyPortal/
   Structure: /Ideology Studio/IdeologyPortal/{ClientName}/{Subfolder}/{filename}
   Client folders are created automatically by Dropbox on first upload.
══════════════════════════════════════════ */
const DBX_ROOT = '/Ideology Studio/IdeologyPortal';
...
function toggleFmtDropdown() {
  ...
  accs.forEach((acc, ai) => {
    ...
    label.innerHTML = `<span style="font-weight:${isActive?700:500};">${esc(acc.name)}</span><span style="color:var(--text-3);font-size:10px;margin-left:6px;">${fmt.label.split('·')[1]?.trim()||''}</span>`;
    ...
    for(const f of filesArr){
      ...
      if(sharedUrl){
        ...
        if(match>=0){
          ...
          console.warn('[Ideology] match=-1 for', f.name, fallback>=0 ? '— fixed via name fallback' : '— item deleted during upload, URL in Dropbox');
        }
...
const TAB_TO_MACRO = {
  stratoverview:'strategia',audit:'strategia',pdm:'strategia',pdc:'strategia',stratbuild:'strategia',brandportal:'strategia',pilastri:'strategia',documenti:'strategia',
  notes:'studio',shooting:'studio',brief:'studio',brand:'studio',mood:'studio',
...
function switchTab(tab, preserveExpanded){
  currentTab=tab;
  routerPush(tab);
  const allTabs=['studio','notes','shooting','brief','brand','pilastri','storyboard','feed','stories','ped','cal','anno','preview','ads','landing','stampa','mood','report','qbr','audit','pdm','pdc','stratbuild','brandportal','stratoverview','documenti'];
  allTabs.forEach(t=>{
  ...
  if(tab==='brief'){renderBriefTab();}
  if(tab==='documenti'){renderDocumentiTab();}
...
// Week Plan è ora l'unico posto dove si crea/gestisce un cliente — questi
// due bottoni ("+ Aggiungi" nell'header Studio, "+ Aggiungi primo cliente"
// nello stato vuoto) aprivano il modale di creazione locale a Content.
// ── Modale "Aggiungi cliente a Content" — due tab, stesso schema di
// Strategia (vedi src/pages/strategia/index.html, funzione modale + helper
// strategyActivateExisting): "Cliente nuovo" crea un cliente condiviso da
// zero con guardia anti-duplicato (già in addClient(), sotto, invariata);
// "Cliente esistente" attiva Content su un cliente già presente in
// Week/Strategia aggiungendogli un account, senza toccare nessun altro
// campo. Sostituisce il redirect a Week Plan introdotto prima in questa
// stessa sessione — su richiesta esplicita dell'utente, dopo aver visto che
// Strategia ha già questo identico pattern e funziona bene lì.
let caAddMode = 'nuovo';
function caSetAddMode(mode){
  caAddMode = mode;
  const btnNuovo = document.getElementById('ca-tab-btn-nuovo');
  const btnEsist = document.getElementById('ca-tab-btn-esistente');
  const panelNuovo = document.getElementById('ca-tab-nuovo');
  const panelEsist = document.getElementById('ca-tab-esistente');
  const accSection = document.getElementById('ca-account-section');
  const divider = document.getElementById('ca-divider');
  const submitBtn = document.getElementById('ca-submit-btn');
  if(btnNuovo) btnNuovo.className = 'btn sm' + (mode==='nuovo' ? ' primary' : ' ghost');
  if(btnEsist) btnEsist.className = 'btn sm' + (mode==='esistente' ? ' primary' : ' ghost');
  if(panelNuovo) panelNuovo.style.display = mode==='nuovo' ? '' : 'none';
  if(panelEsist) panelEsist.style.display = mode==='esistente' ? '' : 'none';
  // In modalità "nuovo" non si crea più nulla qui — solo un invito a Week
  // Plan (unica fonte per il profilo cliente) — quindi account principale
  // e submit non hanno senso e restano nascosti.
  if(accSection) accSection.style.display = mode==='esistente' ? '' : 'none';
  if(divider) divider.style.display = mode==='esistente' ? '' : 'none';
  if(submitBtn) submitBtn.style.display = mode==='esistente' ? '' : 'none';
  if(mode==='esistente') caLoadExistingClients();
}
async function caLoadExistingClients(){
  const sel = document.getElementById('nc-existing-client');
  if(!sel) return;
  sel.innerHTML = '<option value="">Caricamento…</option>';
  try{
    const res = await fetch('/api/clients', { headers:{ 'x-ideology-key':CLOUD.apiKey }, credentials:'include' });
    if(!res.ok) throw new Error('HTTP '+res.status);
    const { clients: allClients } = await res.json();
    // Esclude chi ha già Content attivo (stessa regola di normalizeClient in
    // api/clients.js: flag esplicito, o già almeno un account) — altrimenti
    // ricompaiono clienti già attivi i cui account verrebbero duplicati.
    const available = (allClients||[]).filter(c => !(c.contentActive===true || (c.accounts&&c.accounts.length>0)));
    sel.innerHTML = '<option value="">— seleziona —</option>' +
      available.map(c => '<option value="'+esc(c.id)+'">'+esc(c.name)+'</option>').join('');
    if(!available.length) sel.innerHTML = '<option value="">Nessun cliente disponibile</option>';
  }catch(e){
    console.warn('[caLoadExistingClients] fallito:', e.message);
    sel.innerHTML = '<option value="">Errore di caricamento</option>';
  }
}
async function addClientExisting(){
  const sel = document.getElementById('nc-existing-client');
  const clientId = sel?.value;
  if(!clientId){ showToast('Seleziona un cliente', 'warn'); return; }
  const accName = (document.getElementById('nc-acc-name')?.value.trim()) || '';
  const accPlatform = (document.getElementById('nc-acc-platform')?.value) || 'Instagram';
  const submitBtn = document.getElementById('ca-submit-btn');
  if(submitBtn){ submitBtn.disabled = true; submitBtn.textContent = 'Attivazione…'; }
  try{
    // GET-merge-POST diretto su /api/project, non passa da autoSave()/clients
    // in memoria — il cliente scelto potrebbe non essere nella lista locale
    // (se l'utente è ristretto ad alcuni feed), stesso motivo per cui
    // Strategia fa la stessa identica cosa in strategyActivateExisting().
    const res = await fetch(CLOUD.apiUrl + '?user=' + CLOUD.user, { headers:{ 'x-ideology-key':CLOUD.apiKey }, credentials:'include' });
    if(!res.ok) throw new Error('Lettura fallita: ' + res.status);
    const { data } = await res.json();
    const pd = data || {};
    if(!pd.clients) pd.clients = [];
    const c = pd.clients.find(x => x.id === clientId);
    if(!c) throw new Error('Cliente non trovato nella fonte condivisa.');
    if(c.contentActive === true || (c.accounts && c.accounts.length > 0)){
      throw new Error('Questo cliente ha già Content attivo.');
    }
    if(!c.accounts) c.accounts = [];
    c.accounts.push({ id:genId('a_'), name: accName || c.name, platform: accPlatform, profileImg:'', bio:'' });
    const postRes = await fetch(CLOUD.apiUrl, {
      method:'POST', headers:{ 'Content-Type':'application/json', 'x-ideology-key':CLOUD.apiKey },
      credentials:'include', body: JSON.stringify({ user: CLOUD.user, data: pd }),
    });
    if(!postRes.ok) throw new Error('Salvataggio fallito: ' + postRes.status);
    closeModal('add-client-modal');
    showToast('✓ Content attivato per ' + c.name);
    await loadFromCloud();
  }catch(e){
    showToast('⚠ ' + e.message, 'warn');
  }finally{
    if(submitBtn){ submitBtn.disabled = false; submitBtn.textContent = 'Attiva Content'; }
  }
}
function showStudioAdd(){ caSetAddMode('nuovo'); openModal('add-client-modal'); }
function backToClients(){switchTab('studio');}

/* CLIENT MANAGEMENT */
function openAddClientModal(){ caSetAddMode('nuovo'); openModal('add-client-modal'); }
function addClient(){
  ...
  const defaultAccount={id:genId('a_'),name:accName,platform:accPlatform,profileImg:'',bio:''};
...
function addAccount(){const ci=parseInt(document.getElementById('na-client').value);if(isNaN(ci)||ci<0){showToast('Seleziona un cliente','warn');return;}const name=document.getElementById('na-name').value.trim();if(!name){document.getElementById('na-name').focus();return;}const platform=document.getElementById('na-platform').value;const id=genId('a_');clients[ci].accounts.push({id,name,platform});document.getElementById('na-name').value='';renderStudio();rebuildAllSelects();showToast('✓ Account aggiunto');autoSave();}
// Cancellazione cliente spostata in Week Plan (che possiede l'anagrafica
// cliente) — copre sia il bottone nella tabella Studio sia quello dentro
// il modale di modifica (ecDeleteClient chiama questa stessa funzione).
// Prima di essere disattivata, questa funzione puliva SOLO feeds/stories/
// highlights/pedPlans/notesData — lasciando orfane ideologyDocs,
// ugcInfluencer, pilastri, formati, sbBozze e tutte le funzionalità più
// recenti indicizzate per id. La nuova cancellazione in Week Plan
// (/api/clients DELETE) pulisce tutto in un solo passaggio atomico.
function removeClient(i){
  showToast('Per eliminare un cliente usa Week Plan → Clienti', 'warn');
  window.open('https://nassagestione.com/weekplan/clienti', '_blank');
}
...
function renderStudio(){
  const active=clients.filter(c=>c.status==='Attivo');const totalRev=active.reduce((s,c)=>s+(c.revenue||0),0);const totalAccounts=clients.reduce((s,c)=>s+(c.accounts?.length||0),0);const el=v=>document.getElementById(v);
    ...
    groups.forEach(({month, items})=>{
      ...
      items.forEach(item=>{
        ...
        if(item.needsReload&&!item.url){
          ...
          inner.innerHTML=`<div style="font-size:11px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;opacity:.45;margin-bottom:6px;">${esc(item.editorialEyebrow||'')}</div><div style="font-weight:800;line-height:1.1;letter-spacing:-1px;font-size:17px;flex:1;">${esc(item.editorialTitle||'')}</div>`;
        ...
        const _fc={bozza:{dot:'#6e6e6e',label:'[··] BOZZA'},revisione:{dot:'#ff5a7a',label:'[!!] REVISIONE'},approvare:{dot:'#f59e0b',label:'[??] ATTESA'},approvato:{dot:'#F2C700',label:'[OK] APPROVATO'},pubblicato:{dot:'#35c9d6',label:'[>>] ONLINE'}}[_fst]||{dot:'#6e6e6e',label:'[··] BOZZA'};
        const sb=document.createElement('button');sb.className='feed-cell-stato-btn';sb.title=_fc.label;
        ...
        if(showAllPilastro && item.pilastro){
          ...
          _pilBadge.style.cssText = `position:absolute;top:8px;left:8px;z-index:5;display:inline-flex;align-items:center;gap:4px;padding:2px 8px 2px 5px;border-radius:0;font-size:10px;font-weight:700;background:${_pColor};color:${_pText};pointer-events:none;box-shadow:0 1px 4px rgba(0,0,0,.2);`;
        ...
        if(showAllFormato && item.formato){
          ...
          _fmtBadge.style.cssText = `position:absolute;top:8px;right:8px;z-index:5;display:inline-flex;align-items:center;gap:4px;padding:2px 8px 2px 5px;border-radius:0;font-size:10px;font-weight:700;background:${_fmtColor};color:${_fmtText};pointer-events:none;box-shadow:0 1px 4px rgba(0,0,0,.25);max-width:calc(100% - 60px);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;`;
          _fmtBadge.innerHTML = `<span style="width:5px;height:5px;border-radius:0;background:${_fmtText};opacity:.45;display:inline-block;flex-shrink:0;"></span>${esc(item.formato)}`;
    ...
    if(i<items.length){
      ...
      const coverUrl=item.type==='carousel'&&item.slides?.length?(item.slides[0].url||item.slides[0].externalUrl||''):_itemUrl;
        ...
        if(item.type==='editorial'){
          ...
          // FIX SECURITY: titolo/accento sono testo digitato dall'utente e salvato
          // nel blob condiviso — vanno escapati PRIMA di essere ricomposti in HTML,
          // altrimenti chiunque può salvare <img onerror=...> come titolo e farlo
          // eseguire nel browser di ogni altro membro del team che apre il calendario.
          // Si scappa title/accent, poi si fa l'evidenziazione sulle versioni escapate
          // (l'escaping preserva le relazioni di sottostringa, quindi il replace funziona identico).
          const _rawTitle2363=item.editorialTitle||'', _rawAccent2363=item.editorialAccent||'';
          const titleHtml=_rawAccent2363&&_rawTitle2363.includes(_rawAccent2363)
            ?esc(_rawTitle2363).replace(esc(_rawAccent2363),`<span style="color:${cols.accent};">${esc(_rawAccent2363)}</span>`)
            :esc(_rawTitle2363);
        ...
        if((item.collabAccIds||[]).length>0||item._collabFrom){
          ...
          cb.style.cssText='position:absolute;top:30px;left:6px;background:rgba(0,168,90,.92);color:#fff;font-size:9px;font-weight:700;padding:2px 6px;border-radius:0;z-index:4;display:flex;align-items:center;gap:3px;pointer-events:none;letter-spacing:.02em;';
        ...
        const _fst=item.apprStato||'bozza';
        const _fstCfg={bozza:{dot:'#6e6e6e',label:'[··] BOZZA'},revisione:{dot:'#ff5a7a',label:'[!!] REVISIONE'},approvare:{dot:'#f59e0b',label:'[??] ATTESA'},approvato:{dot:'#F2C700',label:'[OK] APPROVATO'},pubblicato:{dot:'#35c9d6',label:'[>>] ONLINE'}};
        ...
        if((showAllPilastro && item.pilastro) || (showAllFormato && item.formato)){
          ...
          if(showAllPilastro && item.pilastro){
            ...
            _pilBadge2.style.cssText = `display:inline-flex;align-items:center;gap:5px;padding:3px 7px;border-radius:0;font-size:9px;font-weight:600;letter-spacing:.03em;text-transform:uppercase;background:rgba(8,8,8,.84);backdrop-filter:blur(6px);color:#e8e8e8;max-width:100px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex-shrink:1;`;
            _pilBadge2.innerHTML = `<span style="width:5px;height:5px;border-radius:0;background:${_pColor2};opacity:1;display:inline-block;flex-shrink:0;"></span>${esc(item.pilastro)}`;
          ...
          if(showAllFormato && item.formato){
            ...
            _fmtBadge2.style.cssText = `display:inline-flex;align-items:center;gap:5px;padding:0;border-radius:0;font-size:10px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;background:none;color:var(--text-3);max-width:100px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex-shrink:1;`;
            _fmtBadge2.innerHTML = '<span style="width:5px;height:5px;border-radius:0;background:'+_fmtColor2+';opacity:1;display:inline-block;flex-shrink:0;"></span>'+esc(item.formato);
...
/* ══════════════════════════════════════════════════════════════
   ...
   Storage: ideologyDocs[clienteName] = { folders:[], docs:[] }
   Completamente separato da notesData (che rimane intatto)
══════════════════════════════════════════════════════════════ */
let ideologyDocs = {};           // { [clienteName]: { folders:[], docs:[] } }
...
function _docsClientData(){
  ...
  const cl = clients[ci];
  if(!cl) return null;
  if(!ideologyDocs[cl.name]) ideologyDocs[cl.name] = { folders:[], docs:[] };
  return ideologyDocs[cl.name];
}
  ...
  if(storiesAccountIdx<0){const em=document.createElement('div');em.style.cssText='grid-column:1/-1;text-align:center;padding:40px 0;color:var(--text-3);font-size:12px;';em.textContent='📱 Seleziona cliente e account per gestire le stories.';grid.appendChild(em);}
    ...
    if(pedItems.length>0){
      ...
      pedItems.forEach((st,pi)=>{
        ...
        cell.title=(st.type==='autonoma'?'Autonoma':'Template Ideology')+(st.brief?' — '+st.brief:'');
...
function renderSbThumbs(){
  ...
  sbTmpSlides.forEach((sl,i)=>{
    ...
    d.innerHTML='<div class="sb-thumb-num">'+(sl.num||i+1+'.')+'</div><div class="sb-thumb-title">'+esc(sl.title||'Senza titolo')+'</div>';
...
function renderPreview(){
  ...
  const header=document.createElement('div');header.className='preview-header';header.innerHTML=`<div class="preview-logo">${esc(((cl.name||'?').trim()[0]||'?').toUpperCase())}</div><div class="preview-htext"><h2>${esc(cl.name)}</h2><p>${esc(month)}</p></div><div class="preview-hmeta"><span class="preview-chip" id="preview-chip"></span></div>`;body.appendChild(header);
  ...
  if(!ready.length){const em=document.createElement('div');em.className='preview-empty';em.innerHTML='<p>Nessun contenuto per '+esc(acc.name)+' — '+esc(month)+'.</p>';body.appendChild(em);}
  else{
    ...
    ready.forEach((item,i)=>{
      ...
      if(item.type==='video'){
        ...
        try{
         ...
         cell.style.background=cols.bg;cell.style.color=cols.text;
         // FIX SECURITY: vedi nota identica sopra — escape prima di ricomporre l'HTML.
         const _rawTitle5094=item.editorialTitle||'', _rawAccent5094=item.editorialAccent||'';
         const edTitleHtml=_rawAccent5094&&_rawTitle5094.includes(_rawAccent5094)
           ?esc(_rawTitle5094).replace(esc(_rawAccent5094),`<span style="color:${cols.accent};">${esc(_rawAccent5094)}</span>`)
           :esc(_rawTitle5094);
         const cardInner=document.createElement('div');
      ...
      const APPR_CFG = {
        ...
        revisione:   {label:'[!!] REVISIONE', dot:'#ff5a7a', bg:'rgba(224,92,0,0.82)',   text:'#fff0e8',  border:'rgba(224,92,0,0.9)'},
        approvare:   {label:'[??] ATTESA',   dot:'#f5c800', bg:'rgba(212,168,0,0.82)', text:'#3d2e00',  border:'rgba(212,168,0,0.9)'},
        approvato:   {label:'[OK] APPROVATO',    dot:'#F2C700', bg:'rgba(26,122,74,0.82)', text:'#d6fff0',  border:'rgba(26,122,74,0.9)'},
        pubblicato:  {label:'[>>] ONLINE',   dot:'#60a5fa', bg:'rgba(37,99,235,0.82)',  text:'#dbeafe', border:'rgba(37,99,235,0.9)'},
      };
  ...
  if(storyboards.length){
    ...
    storyboards.forEach((sb, si) => {
      ...
      info.innerHTML = `<span class="sb-player-name">${esc(sb.name||'Storyboard')}</span>`
        + (sb.note ? `<span class="sb-player-note">${esc(sb.note)}</span>` : '');
      ...
      function sbRefresh(){
        ...
        const STATO_S = {
          ...
          approvare: {dot:'#f59e0b',label:'Da approvare',bg:'rgba(212,168,0,0.18)',   text:'#7a5c00'},
          approvato: {dot:'#1a7a4a',label:'[OK] APPROVATO',   bg:'rgba(26,122,74,0.14)',   text:'#0f5230'},
        };
  ...
  const footer=document.createElement('div');footer.className='preview-footer';footer.innerHTML='<p>Anteprima preparata da</p><div class="nassa-sig">Ideology Studio · nassastudio.it</div>';body.appendChild(footer);
  ...
  if(item.type==='carousel'&&item.slides?.length){
    ...
    if(url){
      ...
      img.onerror=()=>{
        ...
        ph.innerHTML='<div style="font-size:32px;">🖼</div>'+(item.name?'<div style="font-size:13px;font-weight:600;color:#aaa;">'+esc(item.name)+'</div>':'')+'<div style="font-size:11px;color:#555;">Immagine non disponibile<br>Carica su Dropbox dalla tab Stories</div>';
...
const UGC_STATI = {
  ...
  approvato:  {label:'[OK] APPROVATO',  dot:'var(--ugc-approvato-border)',  bg:'var(--ugc-approvato-bg)',  text:'var(--ugc-approvato-text)', border:'var(--ugc-approvato-border)'},
  ...
  if(sl._inflOpen){
    ...
    if(sl.influencerId){
      ...
      if(inf){
        ...
        badge.style.cssText='display:inline-flex;align-items:center;gap:4px;padding:2px 8px;border-radius:0;font-size:var(--fs-xs);font-weight:600;background:'+sc.bg+';color:'+sc.t+';margin-top:6px;';
...
function pedRenderCollabPanel(){
  ...
  filtered.forEach(inf=>{
    ...
    const bdg=document.createElement('span');bdg.style.cssText=`padding:2px 8px;border-radius:0;font-size:10px;font-weight:700;background:${sc.bg};color:${sc.t};`;bdg.textContent=sc.l;top.appendChild(bdg);
...
function _renderSbTabGridReal(){
  ...
  items.forEach(sb=>{
    ...
    const steps=[
      ...
      {label:'[>>] ONLINE',done:!!sb.fileCaricato}
    ];
...
function renderEcAccounts(){
  ...
  ecTmpAccounts.forEach((acc,i)=>{
    ...
    const nameInp=document.createElement('input');nameInp.className='ec-acc-name-inp';nameInp.value=acc.name||acc.handle||'';nameInp.placeholder='Nome account';nameInp.oninput=e=>{ecTmpAccounts[i].name=e.target.value;ecTmpAccounts[i].handle=e.target.value;};
...
function ecAddAccount(){
  ...
  ecTmpAccounts.push({id:genId('a_'),name,handle:name,platform});
...
function ecSave(){
  ...
  // Nome, pacchetto, stato, revenue non si scrivono più da qui — i campi nel
  // modale sono readonly/disabled, restano quello che erano; li gestisce
  // Week Plan. cl.name non cambia mai da questa funzione, quindi non serve
  // più nessuna logica di "rename" su pedPlans/notesData qui dentro.
...
function ecDeleteClient(){
  if(ecEditIdx<0)return;
  // Niente più dialogo di conferma "Elimina definitivamente" qui — sarebbe
  // fuorviante, dato che il click non cancella nulla ma reindirizza soltanto.
  removeClient(ecEditIdx);
}
...
function loadProjectFile(input){
  ...
  reader.onload=e=>{
    try{
      ...
      clients=data.clients||[];clients.forEach(c=>{if(!c.accounts)c.accounts=[];if(!c.id)c.id=genId('c_');});
...
async function switchUser(username){CLOUD.user=username;localStorage.setItem('ideology_user',username);const av=document.getElementById('user-avatar');if(av)av.textContent=username.slice(0,2).toUpperCase();document.getElementById('user-switcher')?.classList.remove('open');await loadFromCloud();}

// Progetti/feed assegnati — stessa regola di Week: admin/manager vedono
// sempre tutto, un membro con assegnazioni specifiche vede solo quelle.
// Cache per evitare di rifare la fetch a /api/team a ogni loadFromCloud().
let _myAssignmentCache = null;
...
async function loadFromCloud(){
  ...
  if(result?.data){
    ...
    // Content ora si "attiva" per un cliente da Week Plan invece di poterlo
    // creare qui — un cliente compare in Studio solo se contentActive è
    // esplicito, oppure (per tutti i clienti creati prima di questa
    // modifica) se ha già almeno un account social collegato. Cosi nessun
    // cliente con contenuti reali già caricati sparisce da questa lista:
    // l'attivazione esplicita serve solo per i clienti NUOVI, creati da
    // Week Plan senza ancora nessun account.
    clients = clients.filter(c => c.contentActive === true || (Array.isArray(c.accounts) && c.accounts.length > 0));

    // Filtra i clienti visibili in base all'assegnazione — a differenza di
    // Week, qui serve anche tagliare accounts[] per chi ha solo SPECIFICI
    // feed di un cliente assegnati (non l'intero cliente via __all__),
    // perché Content mostra le viste per singolo account, non solo il nome
    // del cliente in un menu.
    const mine = await getMyAssignment();
    if (mine && mine.role !== 'admin' && mine.role !== 'manager' && mine.assignedFeeds.length > 0) {
      const allowedClientIds = new Set(mine.assignedFeeds.map(k => k.split('::')[0]));
      const wholeClientIds = new Set(mine.assignedFeeds.filter(k => k.endsWith('::__all__')).map(k => k.split('::')[0]));
      const allowedAccountIds = new Set(mine.assignedFeeds.filter(k => !k.endsWith('::__all__')).map(k => k.split('::')[1]));
      clients = clients.filter(c => allowedClientIds.has(c.id)).map(c => {
        if (wholeClientIds.has(c.id)) return c; // intero cliente assegnato — accounts intatti
        return { ...c, accounts: (c.accounts||[]).filter(a => allowedAccountIds.has(a.id)) };
      });
    }
  ...
  try {
    ...
    if(oldBozze.length && !Object.keys(sbBozze).length){
      ...
      autoSave();
      console.log('[Ideology] Migrate: '+oldBozze.length+' bozze da localStorage → Supabase');
    }
  } catch(_){}
...
function showBootOverlay(show){
  ...
  if(show){
    if(!ov){
      ...
      const logo=document.createElement('img');logo.src='/assets/favicon.svg';logo.alt='Ideology Studio';logo.style.cssText='width:48px;height:48px;border-radius:10px;object-fit:cover;';
...
function renderEdPreview(){
  ...
  // FIX SECURITY: titleRaw/accentWord arrivano direttamente da campi di input
  // testuali — escape prima di ricomporre l'HTML dell'anteprima (stessa nota
  // delle altre due occorrenze di questo pattern più sopra nel file).
  const titleHtml=accentWord&&titleRaw.includes(accentWord)
    ?esc(titleRaw).replace(esc(accentWord),`<span style="color:${cols.accent};">${esc(accentWord)}</span>`)
    :esc(titleRaw);
  ...
  card.innerHTML=`
    <div class="ecp-eyebrow" style="color:${cols.text};">${esc(eyebrow)}</div>
    <div class="ecp-title" style="font-size:${titleSize};flex:1;">${titleHtml}</div>
    <div class="ecp-div"></div>
    <div class="ecp-copy" style="font-size:${copySize};">${esc(copy)}</div>
    <div class="ecp-footer">
      <div class="ecp-author">${esc(author)}</div>
...
function renderPilastriContent(body,ci){
  ...
  pils.forEach(p=>{
    ...
    chip.innerHTML=`<span class="ps-dot" style="background:${p.color}"></span><span class="ps-name">${esc(p.name)}</span><span class="ps-count">${count} post (${pct}%)</span>`;
  ...
  pils.forEach((p,pi)=>{
    ...
    if(pillarPosts.length){
      ...
      pillarPosts.slice(0,6).forEach(it=>{
        ...
        const _url=it.url||it.externalUrl||'';
        const coverUrl=it.type==='carousel'&&it.slides?.[0]?(it.slides[0].url||it.slides[0].externalUrl||''):it.type!=='video'?_url:'';
        if(it.type==='video'&&_url){const v=document.createElement('video');v.src=_url;v.preload='metadata';if(it.coverUrl)v.setAttribute('poster',it.coverUrl);v.muted=true;v.playsInline=true;v.style.cssText='width:100%;height:100%;object-fit:cover;border-radius:3px;pointer-events:none;';th.appendChild(v);}
        else if(coverUrl){const img=document.createElement('img');img.src=coverUrl;img.alt='';img.style.cssText='width:100%;height:100%;object-fit:cover;border-radius:3px;';th.appendChild(img);}
        else{th.style.background='var(--surface-lt,#2a2a2a)';}
        // Click to unassign
  ...
  addCard.onclick=()=>{
    pils.push({id:genId('p_'),name:'Nuovo pilastro',color:PILASTRI_COLORS[pils.length%PILASTRI_COLORS.length],description:'',postIds:[]});
...
// ── Webhook Ideology Studio → Plan ───────────────────────────────────────────────
// Fire-and-forget: notifica weekplan quando cambia apprStato di un item.
// weekplan fa polling leggero (3 min) e aggiorna i badge fromContentStato.
function notifyContentWebhook(item, oldStato) {
  if (!item || !item.id) return;
  const newStato = item.apprStato || 'bozza';
  if (oldStato === newStato) return; // nessun cambio reale

  // Recupera accountId dal contesto corrente del feed
  const aid = accountId(feedClientIdx, feedAccountIdx) || null;

  fetch('/api/content-webhook', {
    method:      'POST',
    credentials: 'include',
    ...
    // Silenzioso — non blocca il flusso Ideology Studio
    console.warn('[content-webhook] notify failed:', err.message);
  });
}
...
// Richiamo in sola lettura del modulo Funnel di Strategia (c.funnel) dentro
// il modale campagna Ads — richiesto esplicitamente dall'utente, per capire
// a colpo d'occhio la fase target di una campagna senza uscire da Content.
// c.funnel arriva già popolato in clients[] (stesso blob condiviso con
// Strategia, nessun filtro sui campi in loadFromCloud) — verificato prima
// di costruire questa funzione.
const ADS_FUNNEL_LABELS = {
  tofu: { lbl:'TOFU — Consapevolezza', color:'#185FA5' },
  mofu: { lbl:'MOFU — Considerazione', color:'#C2185B' },
  bofu: { lbl:'BOFU — Decisione', color:'#1A8C3F' },
};
// Stessa lista campi di FUNNEL_FIELDS in Strategia (src/pages/strategia/
// index.html) — solo 6 campi per fase, basso rischio di disallineamento
// a differenza di PDM/PDC (18/15 campi ciascuno, per quelli si usa
// l'etichetta generica dalla chiave invece di duplicare qui la lista).
const FUNNEL_FIELDS = [
  {key:"obiettivo", lbl:"Obiettivo della fase"},
  {key:"target", lbl:"Comportamento del target"},
  {key:"formati", lbl:"Formati di contenuto consigliati"},
  {key:"messaggio", lbl:"Messaggio chiave"},
  {key:"kpi", lbl:"KPI di questa fase"},
  {key:"frizione", lbl:"Cosa blocca il passaggio alla fase successiva"},
];
function adsRenderFunnelRef(){
  const sel = document.getElementById('adm-funnel-stage');
  const box = document.getElementById('adm-funnel-ref');
  if(!sel || !box) return;
  const stage = sel.value;
  if(!stage){ box.style.display='none'; box.innerHTML=''; return; }
  const cl = globalClientIdx>=0 ? clients[globalClientIdx] : null;
  const f = (cl && cl.funnel && cl.funnel[stage]) || {};
  const info = ADS_FUNNEL_LABELS[stage];
  if(!f.obiettivo && !f.messaggio && !f.formati){
    box.style.display='block';
    box.innerHTML = `<span style="color:var(--text-3);font-style:italic;">Fase ${esc(info.lbl)} non ancora definita in Strategia — vai su Strategia → ${cl?esc(cl.name):'cliente'} → Funnel per compilarla.</span>`;
    return;
  }
  box.style.display='block';
  box.innerHTML = `
    <div style="font-weight:600;color:${info.color};margin-bottom:4px;">${esc(info.lbl)}</div>
    ${f.obiettivo?`<div style="margin-bottom:3px;"><strong>Obiettivo:</strong> ${esc(f.obiettivo)}</div>`:''}
    ${f.messaggio?`<div style="margin-bottom:3px;"><strong>Messaggio:</strong> ${esc(f.messaggio)}</div>`:''}
    ${f.formati?`<div><strong>Formati consigliati:</strong> ${esc(f.formati)}</div>`:''}
  `;
}
    ...
    return `<div class="ads-camp">
      ...
      <div style="flex:1;min-width:0;">
        ...
        <div class="ads-camp-meta">
          ...
          ${camp.funnelStage?`<span style="font-size:10px;font-weight:700;padding:2px 7px;border-radius:0;background:${ADS_FUNNEL_LABELS[camp.funnelStage].color}22;color:${ADS_FUNNEL_LABELS[camp.funnelStage].color};">${camp.funnelStage.toUpperCase()}</span>`:''}
...
function openEditAdsCampaign(id){
  ...
  el('adm-funnel-stage').value=camp.funnelStage||'';
  adsRenderFunnelRef();
  // Load creative
  adsClearCreative();
  if(camp.creativeUrl){
...
async function saveAdsCampaign(){
  ...
  if(_adsCreativeFile){
    ...
    try{
      ...
      const res=await fetch('/api/dropbox-upload',{
        method:'POST',
        headers:{'x-ideology-key':CLOUD.apiKey},
        credentials:'include',
        body:formData
      });
      if(res.ok){
  ...
  const entry={
    ...
    funnelStage:document.getElementById('adm-funnel-stage')?.value||'',
  ...
  Object.keys(pedPlans).forEach(k => {
    ...
    (pedPlans[k] || []).forEach(item => {
      ...
      result[mi].push({
        ...
        sub: item.type === 'autonoma' ? 'Autonoma' : 'Template Ideology',
        tag: 'UGC',
        raw: item
      });
    });
...
function renderAnnoTab() {
  ...
  if(footer) {
    ...
    footer.innerHTML = `
      <span class="anno-footer-txt">IDEOLOGY STUDIO · ${cln} · ${annoYear}</span>
...
const APPR_STATI = [
  ...
  {key:'revisione',   label:'[!!] REVISIONE',   dot:'#ff5a7a', bg:'rgba(224,92,0,0.13)',    text:'#7a2e00',       border:'rgba(224,92,0,0.45)'},
  {key:'approvare',   label:'[??] ATTESA',     dot:'#f59e0b', bg:'rgba(212,168,0,0.15)',   text:'#7a5c00',       border:'rgba(212,168,0,0.5)'},
  {key:'approvato',   label:'[OK] APPROVATO',        dot:'#1a7a4a', bg:'rgba(26,122,74,0.12)',   text:'#0f5230',       border:'rgba(26,122,74,0.4)'},
  {key:'pubblicato',  label:'[>>] ONLINE',       dot:'#35c9d6', bg:'rgba(37,99,235,0.12)',   text:'#1e3a8a',       border:'rgba(37,99,235,0.4)'},
];
...
function apprChangeStato(idx, stato){
  ...
  notifyContentWebhook(items[idx], oldStato); // → weekplan sync
  apprOpenMenu = null;
  autoSave();
  renderPreview();
}
...
function apprModalApprova(){
  if(apprModalIdx===null) return;
  const item    = apprModalItems[apprModalIdx];
  const oldStat = item ? (item.apprStato || 'bozza') : 'bozza';
  apprModalSalva(true);
  refreshFeed();
  apprModalSetStato('approvato');
  if(item) item.apprStato = 'approvato';
  notifyContentWebhook(item, oldStat); // → weekplan sync
  closeApprModal();
  autoSave();
  renderPreview();
}
...
function cycleApprStato(idx, items){
  ...
  notifyContentWebhook(items[idx], cur); // → weekplan sync
  autoSave();
  refreshFeed();
}
...
window.addEventListener('online', async () => {
  ...
  const local = localStorage.getItem('ideology_offline_snapshot');
  if(local){
    try {
      ...
      if(user === CLOUD.user){
        ...
        if(CLOUD._status === 'saved'){
          localStorage.removeItem('ideology_offline_snapshot');
...
══════════════════════════════════════════ */
let _ideologyUser = null; // { username, role } — set after successful auth
let _loginSelected = null; // { username, nome, initials, bg, color } — persona scelta nel picker
...
function _showLoginScreen(errorMsg) {
  ...
  document.getElementById('ideology-login-screen')?.remove();

  const screen = document.createElement('div');
  screen.id = 'ideology-login-screen';
  ...
  _renderLoginHeader(screen);

  const body = document.createElement('div');
  body.id = 'login-body';
  screen.querySelector('#login-card').appendChild(body);

  if (errorMsg) {
    const err = document.createElement('div');
    err.id = 'login-error';
    err.style.cssText = 'background:#fee2e2;color:#991b1b;border-radius:8px;padding:10px 14px;font-size:13px;margin-bottom:16px;';
    err.textContent = errorMsg;
    body.appendChild(err);
  }

  _loginSelected = null;
  _renderLoginPicker(body);
}

function _renderLoginHeader(screen) {
  const wrap = document.createElement('div');
  wrap.style.cssText = 'width:100%;max-width:420px;padding:0 24px;';
  wrap.innerHTML = `
    <div style="text-align:center;margin-bottom:32px;">
      <img src="/assets/favicon.svg" alt="Ideology Studio" style="width:52px;height:52px;border-radius:14px;object-fit:cover;margin:0 auto 14px;display:block;">
      <div style="font-size:22px;font-weight:700;color:var(--text,#111);letter-spacing:-.5px;">Ideology Studio</div>
      <div id="login-subtitle" style="font-size:13px;color:var(--text-3,#888);margin-top:4px;">Chi sei?</div>
    </div>
    <div id="login-card"></div>`;
  screen.appendChild(wrap);
}

// ── Step 1: griglia con avatar/nome di chi ha un account ──────────────────
// Markup avatar per il picker: foto se c'è, altrimenti cerchio iniziali —
// stessa idea di _renderAvatar ma qui serve come stringa HTML perché i
// tile vengono costruiti con innerHTML, non con nodi separati.
function _avatarMarkup(p, sizePx) {
  if (p.avatarUrl) {
    return `<img src="${_esc(p.avatarUrl)}" alt="${_esc(p.nome)}" style="width:${sizePx}px;height:${sizePx}px;border-radius:50%;object-fit:cover;display:block;">`;
  }
  const fontSize = Math.round(sizePx * 0.3);
  return `<div style="width:${sizePx}px;height:${sizePx}px;border-radius:50%;background:${p.bg};color:${p.color};
    display:flex;align-items:center;justify-content:center;font-size:${fontSize}px;font-weight:700;">${_esc(p.initials)}</div>`;
}

async function _renderLoginPicker(body) {
  body.innerHTML = '<div style="text-align:center;color:var(--text-3,#888);font-size:13px;padding:20px 0;">Caricamento…</div>';

  let people = [];
  try {
    const r = await fetch('/api/login-directory');
    if (r.ok) { const d = await r.json(); people = d.people || []; }
  } catch(e) { /* fallback sotto */ }

  if (!people.length) {
    // Nessun account nel picker (rollout iniziale, o errore di rete) —
    // torna al form classico invece di bloccare l'accesso.
    _renderManualLogin(body);
    return;
  }

  const grid = document.createElement('div');
  grid.style.cssText = 'display:grid;grid-template-columns:repeat(auto-fill,minmax(84px,1fr));gap:14px;margin-bottom:8px;';

  people.forEach(p => {
    const tile = document.createElement('button');
    tile.type = 'button';
    tile.style.cssText = 'display:flex;flex-direction:column;align-items:center;gap:6px;background:none;border:none;cursor:pointer;font-family:inherit;padding:6px;border-radius:10px;transition:background .12s;';
    tile.onmouseover = () => tile.style.background = 'rgba(0,0,0,.04)';
    tile.onmouseout = () => tile.style.background = 'none';
    tile.innerHTML = `
      ${_avatarMarkup(p, 52)}
      <div style="font-size:11px;font-weight:500;color:var(--text,#111);text-align:center;max-width:80px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${_esc(p.nome)}</div>`;
    tile.addEventListener('click', () => {
      _loginSelected = p;
      _renderPasswordStep(body);
    });
  ...
  manualLink.textContent = 'Non ti vedi in lista? Accedi con username';
  manualLink.style.cssText = 'display:block;margin:12px auto 0;background:none;border:none;color:var(--text-3,#888);font-size:12px;cursor:pointer;text-decoration:underline;font-family:inherit;';
  manualLink.addEventListener('click', () => _renderManualLogin(body));
  body.appendChild(manualLink);
}

// ── Step 2: password per la persona scelta ─────────────────────────────────
function _renderPasswordStep(body) {
  const p = _loginSelected;
  const sub = document.getElementById('login-subtitle');
  if (sub) sub.textContent = 'Inserisci la password';

  body.innerHTML = `
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;padding:10px;background:var(--surface,#fff);border-radius:10px;border:1px solid var(--border,#eee);">
      ${_avatarMarkup(p, 40)}
      <div style="flex:1;font-size:14px;font-weight:600;color:var(--text,#111);">${_esc(p.nome)}</div>
      <button type="button" id="login-back-btn" title="Non sei tu?"
        style="background:none;border:none;color:var(--text-3,#888);cursor:pointer;font-size:12px;text-decoration:underline;font-family:inherit;">‹ cambia</button>
    </div>
    ...
    <div style="position:relative;margin-bottom:12px;">
      <input id="login-password" type="password" placeholder="Password" autocomplete="current-password"
        style="width:100%;box-sizing:border-box;padding:12px 14px;border:1.5px solid var(--border,#ddd);border-radius:10px;font-size:15px;font-family:inherit;background:var(--surface,#fff);color:var(--text,#111);outline:none;"
        onfocus="this.style.borderColor='#1a8c3f'" onblur="this.style.borderColor=''"
        onkeydown="if(event.key==='Enter')_doLogin()" />
      <button type="button" onclick="const p=document.getElementById('login-password');p.type=p.type==='password'?'text':'password';"
        style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:var(--text-3,#888);font-size:13px;padding:4px;">👁</button>
    </div>
    <button id="login-btn" onclick="_doLogin()"
      style="width:100%;padding:13px;background:#1a8c3f;color:#fff;border:none;border-radius:10px;font-size:15px;font-weight:600;cursor:pointer;font-family:inherit;"
      onmouseover="this.style.background='#157a34'" onmouseout="this.style.background='#1a8c3f'">Accedi</button>`;

  document.getElementById('login-back-btn').addEventListener('click', () => {
    _loginSelected = null;
    if (sub) sub.textContent = 'Chi sei?';
    _renderLoginPicker(body);
  });
  setTimeout(() => document.getElementById('login-password')?.focus(), 50);
}

// ── Fallback: form classico username + password (rollout iniziale / errore) ──
function _renderManualLogin(body) {
  const sub = document.getElementById('login-subtitle');
  if (sub) sub.textContent = 'Accedi al tuo account';
  _loginSelected = null;
  ...
  body.innerHTML = `
    ...
    <div style="display:flex;flex-direction:column;gap:12px;">
      ...
      <button id="login-btn" onclick="_doLogin()"
        style="padding:13px;background:#1a8c3f;color:#fff;border:none;border-radius:10px;font-size:15px;font-weight:600;cursor:pointer;font-family:inherit;margin-top:4px;"
        onmouseover="this.style.background='#157a34'" onmouseout="this.style.background='#1a8c3f'">Accedi</button>
    </div>`;
...
async function _doLogin() {
  // Username: dal picker se una persona è stata selezionata, altrimenti dal campo manuale
  const username = _loginSelected ? _loginSelected.username : document.getElementById('login-username')?.value.trim();
  ...
  try {
    ...
    _ideologyUser = { username: data.username, role: data.role, profile: data.profile };
        // Mostra settings link per admin/manager
        if (data.role === 'admin' || data.role === 'manager') {
          const sl = document.getElementById('settings-menu-link');
          const sd = document.getElementById('settings-menu-divider');
...
async function ideologyLogout() {
  await fetch('/api/auth', { method: 'DELETE', credentials: 'include' });
  _ideologyUser = null;
  _myAssignmentCache = null; // altrimenti sopravvive a un logout→login di un'altra persona
  _showLoginScreen();
}

// Mostra la foto profilo se c'è, altrimenti le iniziali reali (mai le prime
// lettere dello username — per gli id generati tipo "m_1783..." il
// risultato era chiaramente sbagliato, ed erano comunque sbagliate anche
// per gli account normali: "luca" -> "LU" non sono le iniziali vere "LG").
function _renderAvatar(el, profile, username) {
  if (!el) return;
  if (profile && profile.avatarUrl) {
    ...
    img.alt = profile.nome || username || '';
    el.appendChild(img);
  } else {
    el.textContent = (profile && profile.initials) || (username || '').slice(0, 2).toUpperCase();
  }
}
...
async function _bootApp() {
  ...
  if (_ideologyUser) _renderAvatar(av, _ideologyUser.profile, _ideologyUser.username);
  // Run main init
  init();
  await loadFromCloud();
}
  ...
  const updateBar = () => {
    ...
    if(item.pilastro){
      ...
      if(pils.length > 0){
        ...
        sel.style.cssText = 'font-size:9px;border:0.5px dashed var(--border);border-radius:0;padding:1px 4px;background:transparent;color:var(--text-3);font-family:var(--font);cursor:pointer;';
...
function renderFormatiSection(body, ci){
  ...
  if(fmts.length > 0){
    ...
    fmts.forEach(f => {
      ...
      chip.innerHTML = `<span class="ps-dot" style="background:${f.color}"></span><span class="ps-name">${esc(f.name)}</span><span class="ps-count">${count} post (${pct}%)</span>`;
      statsBar.appendChild(chip);
    });
  ...
  addCard.onclick = () => {
    ...
    fmts.push({ id:genId('fmt_'), name:'Nuovo format', color:newColor, description:'' });
    formati[cl.name] = fmts;
    autoSave();
    renderPilastri();
  };
...
/* ══════════════════════════════════════════════════════════
   LANDING PAGE SYSTEM — Ideology Studio
...
const LANDING_STATUS = {
  bozza:      { label:'[··] BOZZA',      dot:'#6e6e6e',    bg:'rgba(100,100,100,.12)', text:'var(--text-2)' },
  ...
  items.forEach((item, idx) => {
    ...
    info.innerHTML = `
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
        ...
        <span style="font-size:10px;font-weight:600;padding:2px 7px;border-radius:0;background:${st.bg};color:${st.text};">${st.label}</span>
...
function saveLanding(){
  ...
  const items = currentLandings();
  const entry = {
    id: _landingEditIdx !== null ? items[_landingEditIdx].id : genId('lp_'),
    name,
    url: g('lm-url'),
    goal: g('lm-goal'),
...
/* ══════════════════════════════════════════════════════════
   CARTELLA STAMPA — Ideology Studio
    ...
    uscite.forEach((item, i) => {
      ...
      info.innerHTML = `
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:3px;flex-wrap:wrap;">
          ...
          <span style="font-size:10px;padding:1px 6px;border-radius:0;background:${TIPO_MEDIA_COLORS[tm]};color:${TIPO_MEDIA_TEXT[tm]};font-weight:500;">${TIPO_MEDIA_LABELS[tm]}</span>
        </div>
...
function saveStampa(){
  ...
  const items = currentStampaItems();
  const entry = {
    id: _stampaEditIdx!==null ? items[_stampaEditIdx].id : genId('st_'),
...
async function triggerMockupUpload(item, idx, sectionEl){
  ...
  input.onchange = async () => {
    ...
    for(const file of files){
      try{
        const cl = globalClientIdx >= 0 ? clients[globalClientIdx] : null;
        const destPath = `/IdeologyStudio/${cl?.name||'Condiviso'}/Landing/${item.name||'mockup'}/${Date.now()}_${file.name}`;
        const fd = new FormData();
        fd.append('file', file);
        fd.append('path', destPath);
        const res = await fetch('/api/dropbox-upload', {
          method: 'POST',
          headers: {'x-ideology-key': ''},
          credentials: 'include',
          body: fd
        });
...
/* ══════════════════════════════════════════════════════════
   MOODBOARD — Ideology Studio
   Griglia masonry 3 colonne: immagini, colori, testi
   Stesso sistema di IdeologyBrand ma in vanilla JS
...
function renderMoodTab(){
  ...
  if(usedTags.length > 0){
    ...
    const renderFilter = () => {
      ...
      allBtn.style.cssText = 'padding:3px 10px;border-radius:0;font-size:11px;cursor:pointer;font-family:var(--font);border:0.5px solid var(--border);background:'+(activeTag===null?'var(--text-1)':'transparent')+';color:'+(activeTag===null?'var(--surface)':'var(--text-2)')+';';
      ...
      usedTags.forEach(tag => {
        ...
        btn.style.cssText = `padding:3px 10px;border-radius:0;font-size:11px;cursor:pointer;font-family:var(--font);border:0.5px solid ${tc};background:${isActive?tc:'transparent'};color:${isActive?'#fff':tc};font-weight:${isActive?'600':'400'};`;
...
function openMoodModal(tipo){
      ...
      <div>
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;" id="mood-tag-btns">
          ${MOOD_TAG_OPTS.map(t => {
            ...
            return `<button onclick="window._moodSetTag('${t}',this)" data-tag="${t}" style="padding:3px 10px;border-radius:0;font-size:11px;border:0.5px solid ${tc};background:transparent;color:${tc};cursor:pointer;font-family:var(--font);">${t}</button>`;
          }).join('')}
        </div>
      </div>
    </div>
  ...
  window._moodHandleFile = async (inp) => {
    ...
    try {
      const cl = globalClientIdx >= 0 ? clients[globalClientIdx] : null;
      const destPath = `/IdeologyStudio/${cl?.name||'Condiviso'}/Moodboard/${Date.now()}_${file.name}`;
      const fd = new FormData();
      fd.append('file', file); fd.append('path', destPath);
      const res = await fetch('/api/dropbox-upload', {
        method:'POST', headers:{'x-ideology-key':''},
...
/* ══════════════════════════════════════════════════════════
   SHOOTING BRIEF — Ideology Studio
   shooting[clientId] = [{
  ...
  items.forEach((item, idx) => {
    ...
    info.innerHTML = `
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
        ...
        <span style="font-size:10px;font-weight:600;padding:2px 7px;border-radius:0;background:${st.bg};color:${st.color};">${st.label}</span>
      </div>
...
function saveShooting(){
  ...
  const items = currentShootings();
  const entry = {
    id: _shootingEditIdx!==null ? items[_shootingEditIdx].id : genId('sh_'),
    nome,
    data: g('sh-data'),
...
// ══════════════════════════════════════════════════════════
// BRIEF (Grafico / Storyboard Video / Naming) — sezione operativa
// richiesta esplicitamente dall'utente, sullo stampo di Shooting Brief
// qui sopra ma parametrizzata per tipo invece di triplicare il codice
// (stesso schema già usato da planFinModal(proj, tipo, item) in
// weekplan per invoice/quote/expense). Shooting Brief NON è stato
// unificato qui dentro — resta nella sua pagina separata, dati e
// funzioni invariati, per non rischiare di toccare qualcosa che
// funziona già. 'video' è la chiave interna per lo storyboard video
// (non 'storyboard' — quella chiave è già usata da un tab esistente
// completamente diverso, le bozze contenuti per account/mese).
// ══════════════════════════════════════════════════════════

const BRIEF_TIPI = {
  grafico:    { label:'Brief Grafico',    icon:'🎨', color:'#a855f7', bg:'rgba(168,85,247,.1)' },
  video:      { label:'Storyboard Video', icon:'🎬', color:'#185FA5', bg:'rgba(24,95,165,.1)' },
  naming:     { label:'Brief Naming',     icon:'✏️', color:'#d97706', bg:'rgba(217,119,6,.1)' },
};

const BRIEF_SEZIONI = {
  grafico: [
    {key:'obiettivo', lbl:'Obiettivo del brief',
     hint:'Cosa deve comunicare questo asset grafico. Dove verrà usato.',
     ph:'Es. Carosello Instagram per il lancio prodotto. 5 slide, tono premium.'},
    {key:'formato', lbl:'Formato e dimensioni',
     hint:'Dove verrà pubblicato o stampato — misure esatte se note.',
     ph:'Es. Feed 1080x1350, storie 1080x1920, oppure A3 per stampa'},
    {key:'copy', lbl:'Contenuti testuali',
     hint:'Il copy esatto da inserire, headline, CTA.',
     ph:'Es. Headline: "Da 40 anni, lo stesso sapore". CTA: "Scoprilo in negozio"'},
    {key:'mood_ref', lbl:'Mood & Reference visivi',
     hint:'Stile, palette, riferimenti a lavori esistenti.',
     ph:'Es. Palette caldi terracotta/crema, no stock photo, illustrazioni a mano'},
    {key:'obbligatori', lbl:'Elementi obbligatori',
     hint:'Cosa deve esserci sempre — logo, claim, disclaimer.',
     ph:'Es. Logo in basso a destra, claim sempre visibile, hashtag campagna'},
    {key:'non_fare', lbl:'Cosa evitare',
     hint:'Vincoli di brand o richieste esplicite del cliente da non violare.',
     ph:'Es. No colori sgargianti, no font diversi da quelli brand'},
    {key:'output', lbl:'Deliverable attesi',
     hint:'Formati file, quante varianti, entro quando.',
     ph:'Es. 3 varianti in PNG + sorgente AI, entro venerdì'},
  ],
  video: [
    {key:'obiettivo', lbl:'Obiettivo del video',
     hint:'Cosa deve comunicare, dove verrà pubblicato.',
     ph:'Es. Reel Instagram per presentare il nuovo servizio. Tono informale.'},
    {key:'formato', lbl:'Durata e formato',
     hint:'Secondi, orientamento, piattaforma di destinazione.',
     ph:'Es. 30-45 secondi, verticale 9:16, per Reels/TikTok'},
    {key:'script', lbl:'Script / scena per scena',
     hint:'Narrazione dettagliata, scena per scena, con testo/voce se previsti.',
     ph:'1. Apertura: primo piano prodotto (2s)\n2. Mani che lavorano (5s)\n3. Testimonianza cliente (10s)\n...'},
    {key:'mood_ref', lbl:'Mood & Reference visivi',
     hint:'Stile di montaggio, ritmo, riferimenti a video esistenti.',
     ph:'Es. Montaggio dinamico stile Reels, tagli veloci, no transizioni elaborate'},
    {key:'audio', lbl:'Musica e audio',
     hint:'Traccia audio desiderata, voce fuori campo sì/no, sottotitoli.',
     ph:'Es. Musica upbeat royalty-free, sottotitoli sempre presenti (audio spento di default)'},
    {key:'non_fare', lbl:'Cosa evitare',
     hint:'Vincoli espliciti da rispettare.',
     ph:'Es. No effetti/filtri vistosi, no musica con copyright'},
    {key:'output', lbl:'Deliverable attesi',
     hint:'Formati file, versioni, scadenza.',
     ph:'Es. MP4 H.264, versione con e senza sottotitoli bruciati, entro 10 giorni'},
  ],
  naming: [
    {key:'obiettivo', lbl:'Cosa deve essere nominato',
     hint:'Prodotto, servizio, linea, brand — e in che contesto verrà usato il nome.',
     ph:'Es. Nuova linea di gelati vegani, da lanciare in estate 2026'},
    {key:'territorio', lbl:'Territorio semantico',
     hint:'Parole chiave, sensazioni, aree di significato da esplorare.',
     ph:'Es. Naturalezza, leggerezza, gioco — evitare riferimenti troppo tecnici'},
    {key:'vincoli', lbl:'Vincoli',
     hint:'Lunghezza, lingua, pronunciabilità, disponibilità dominio/social.',
     ph:'Es. Massimo 2 parole, facile da pronunciare in italiano, dominio .it libero'},
    {key:'riferimenti', lbl:'Naming di riferimento',
     hint:'Nomi che piacciono (anche di altri settori) o da cui prendere ispirazione — e nomi da evitare.',
     ph:'Es. Piace: "Alpro", "Innocent" — Evitare: nomi troppo simili a "Magnum"'},
    {key:'target', lbl:'Target di riferimento',
     hint:'A chi deve parlare il nome — età, stile di vita.',
     ph:'Es. 25-40 anni, attenti all\'alimentazione, urbani'},
    {key:'output', lbl:'Deliverable attesi',
     hint:'Quante proposte, con che livello di motivazione, entro quando.',
     ph:'Es. 8-10 proposte con breve razionale ciascuna, entro una settimana'},
  ],
};
...
function currentBriefs(){
  const k = currentBriefKey();
  return k ? (brief[k] || []) : [];
}
function setBriefs(arr){
  const k = currentBriefKey();
  if(k) brief[k] = arr;
}

let _briefFilterTipo = 'all';
...
function renderBriefTab(){
  ...
  const cl = globalClientIdx >= 0 ? clients[globalClientIdx] : null;
  if(!cl){
    body.innerHTML = '<div style="text-align:center;padding:60px 0;color:var(--text-3);font-size:13px;">Seleziona un cliente per gestire i brief.</div>';
    return;
  }

  const allItems = currentBriefs();
  const items = _briefFilterTipo === 'all' ? allItems : allItems.filter(i => i.tipo === _briefFilterTipo);

  // ── Header ──
  const hdr = document.createElement('div');
  hdr.style.cssText = 'display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;';
  hdr.innerHTML = `<div>
    <h2 style="font-size:20px;font-weight:600;color:var(--text-1);margin-bottom:3px;">Brief</h2>
    <p style="font-size:12px;color:var(--text-3);">${cl.name} — ${allItems.length} brief</p>
  </div>`;
  const addBtn = document.createElement('button');
  addBtn.className = 'btn';
  addBtn.style.cssText = 'background:var(--green);color:#003;border-color:var(--green);font-weight:600;';
  addBtn.textContent = '+ Nuovo brief';
  addBtn.onclick = () => openBriefDetail(null, _briefFilterTipo !== 'all' ? _briefFilterTipo : 'grafico');
  hdr.appendChild(addBtn);
  body.appendChild(hdr);

  // ── Filtro tipo ──
  const filt = document.createElement('div');
  filt.style.cssText = 'display:flex;gap:6px;flex-wrap:wrap;margin-bottom:20px;';
  [['all','Tutti'], ...Object.entries(BRIEF_TIPI).map(([id,t]) => [id, t.icon+' '+t.label])].forEach(([id,lbl]) => {
    const b = document.createElement('button');
    b.textContent = lbl;
    const isAct = _briefFilterTipo === id;
    b.style.cssText = `font-size:11px;font-weight:600;padding:5px 12px;border-radius:0;cursor:pointer;border:.5px solid ${isAct?'var(--text-1)':'var(--border)'};background:${isAct?'var(--text-1)':'transparent'};color:${isAct?'var(--surface)':'var(--text-2)'};`;
    b.onclick = () => { _briefFilterTipo = id; renderBriefTab(); };
    filt.appendChild(b);
  });
  ...
  if(!items.length){
    const em = document.createElement('div');
    em.style.cssText = 'text-align:center;padding:60px 0;color:var(--text-3);';
    em.innerHTML = '<div style="font-size:36px;margin-bottom:12px;">📋</div><div style="font-size:14px;font-weight:500;margin-bottom:6px;">Nessun brief'+(_briefFilterTipo!=='all'?' di questo tipo':'')+'</div><div style="font-size:12px;">Crea il primo brief operativo per chi deve produrre.</div>';
    body.appendChild(em);
    return;
  }
  ...
  items.forEach((item) => {
    const idx = allItems.indexOf(item); // indice reale nell'array completo, non nel filtrato
    const t = BRIEF_TIPI[item.tipo] || BRIEF_TIPI.grafico;
    const card = document.createElement('div');
    card.style.cssText = 'background:var(--surface);border:0.5px solid var(--border);border-radius:10px;padding:14px 16px;display:flex;align-items:center;gap:16px;cursor:pointer;transition:border-color .12s;';
    card.onmouseenter = () => card.style.borderColor = 'var(--border-strong)';
    card.onmouseleave = () => card.style.borderColor = 'var(--border)';
    ...
    const dot = document.createElement('div');
    dot.style.cssText = `width:32px;height:32px;border-radius:8px;background:${t.bg};display:flex;align-items:center;justify-content:center;font-size:15px;flex-shrink:0;`;
    ...
    info.innerHTML = `
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
        <span style="font-size:14px;font-weight:500;color:var(--text-1);">${esc(item.nome||'Brief senza titolo')}</span>
        <span style="font-size:10px;font-weight:600;padding:2px 7px;border-radius:0;background:${t.bg};color:${t.color};">${t.label}</span>
      </div>
      ${item.obiettivo?`<div style="font-size:11px;color:var(--text-3);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:500px;">${esc(item.obiettivo.slice(0,120))}${item.obiettivo.length>120?'…':''}</div>`:''}`;
    card.appendChild(info);
    ...
    editBtn.style.cssText = 'padding:5px 12px;border:0.5px solid var(--border);border-radius:6px;font-size:11px;color:var(--text-2);cursor:pointer;background:transparent;font-family:var(--font);';
    editBtn.textContent = 'Apri';
    editBtn.onclick = e => { e.stopPropagation(); openBriefDetail(idx, item.tipo); };
    acts.appendChild(editBtn);
    card.appendChild(acts);

    card.onclick = () => openBriefDetail(idx, item.tipo);
    list.appendChild(card);
  });
...
let _briefEditIdx = null;
let _briefEditTipo = 'grafico';

function openBriefDetail(idx, tipo){
  _briefEditIdx = idx;
  _briefEditTipo = tipo || 'grafico';
  const items = currentBriefs();
  const item = idx !== null ? items[idx] : {};
  if(idx !== null) _briefEditTipo = item.tipo || _briefEditTipo;
  document.getElementById('brief-detail')?.remove();

  const t = BRIEF_TIPI[_briefEditTipo];
  const sezioni = BRIEF_SEZIONI[_briefEditTipo];

  const overlay = document.createElement('div');
  overlay.id = 'brief-detail';
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:1000;display:flex;align-items:flex-start;justify-content:center;padding:20px;overflow-y:auto;';
  overlay.onclick = e => { if(e.target===overlay) overlay.remove(); };
  ...
  hdrEl.innerHTML = `
    <div>
      <div style="font-size:16px;font-weight:600;color:var(--text-1);">${idx!==null?'Modifica':'Nuovo'} ${t.icon} ${t.label}</div>
      <div style="font-size:11px;color:var(--text-3);margin-top:2px;">Brief operativo per chi deve produrre</div>
    </div>
    <button onclick="document.getElementById('brief-detail').remove()" style="background:none;border:none;cursor:pointer;color:var(--text-3);font-size:22px;line-height:1;">×</button>`;
  box.appendChild(hdrEl);
  ...
  const topRow = document.createElement('div');
  topRow.style.cssText = 'display:grid;grid-template-columns:1fr 180px;gap:12px;';
  topRow.innerHTML = `
    <div>
      <label style="font-size:11px;color:var(--text-3);display:block;margin-bottom:5px;">Nome brief</label>
      <input id="br-nome" type="text" placeholder="Es. ${t.label} — lancio estate" value="${esc(item.nome||'')}"
        style="width:100%;padding:8px 10px;border:0.5px solid var(--border);border-radius:8px;font-size:13px;font-family:var(--font);background:var(--surface-lt);color:var(--text-1);outline:none;box-sizing:border-box;">
    </div>
    <div>
      <label style="font-size:11px;color:var(--text-3);display:block;margin-bottom:5px;">Tipo</label>
      <select id="br-tipo" style="width:100%;padding:8px 10px;border:0.5px solid var(--border);border-radius:8px;font-size:13px;font-family:var(--font);background:var(--surface-lt);color:var(--text-1);outline:none;">
        ${Object.entries(BRIEF_TIPI).map(([id,bt]) => `<option value="${id}" ${id===_briefEditTipo?'selected':''}>${bt.icon} ${bt.label}</option>`).join('')}
      </select>
    </div>`;
  bodyEl.appendChild(topRow);
  topRow.querySelector('#br-tipo').addEventListener('change', (e) => {
    // Cambiare tipo cambia le sezioni del form — salva quel poco che c'è
    // già in nome/note e riapre col set di campi giusto, invece di
    // provare a far quadrare sezioni diverse nello stesso form.
    _briefEditTipo = e.target.value;
    const nome = document.getElementById('br-nome')?.value || '';
    document.getElementById('brief-detail').remove();
    openBriefDetail(idx, _briefEditTipo);
    setTimeout(() => { const n = document.getElementById('br-nome'); if(n) n.value = nome; }, 0);
  });
  ...
  sezioni.forEach(sec => {
    ...
    fieldEl.innerHTML = `
      <label style="font-size:12px;font-weight:500;color:var(--text-1);display:block;margin-bottom:3px;">${esc(sec.lbl)}</label>
      <div style="font-size:11px;color:var(--text-3);margin-bottom:6px;">${esc(sec.hint)}</div>
      <textarea id="br-${sec.key}" rows="3" placeholder="${esc(sec.ph)}"
        style="width:100%;padding:8px 10px;border:0.5px solid var(--border);border-radius:8px;font-size:12px;font-family:var(--font);background:var(--surface-lt);color:var(--text-1);outline:none;resize:vertical;box-sizing:border-box;line-height:1.6;">${esc(item[sec.key]||'')}</textarea>`;
    bodyEl.appendChild(fieldEl);
  });
  ...
  noteEl.innerHTML = `
    <label style="font-size:12px;font-weight:500;color:var(--text-1);display:block;margin-bottom:6px;">Note aggiuntive</label>
    <textarea id="br-note" rows="2" placeholder="Qualsiasi altra nota per il team…"
      style="width:100%;padding:8px 10px;border:0.5px solid var(--border);border-radius:8px;font-size:12px;font-family:var(--font);background:var(--surface-lt);color:var(--text-1);outline:none;resize:vertical;box-sizing:border-box;">${esc(item.note||'')}</textarea>`;
  bodyEl.appendChild(noteEl);
  ...
  footerEl.style.cssText = 'padding:14px 24px;border-top:0.5px solid var(--border);display:flex;justify-content:space-between;align-items:center;position:sticky;bottom:0;background:var(--surface);border-radius:0 0 14px 14px;';
  footerEl.innerHTML = `
    ${idx!==null?`<button onclick="deleteBrief(${idx})" style="padding:7px 14px;border:0.5px solid #f87171;border-radius:8px;font-size:12px;color:#f87171;cursor:pointer;background:transparent;font-family:var(--font);">Elimina</button>`:'<span></span>'}
    <div style="display:flex;gap:8px;">
      <button onclick="document.getElementById('brief-detail').remove()" style="padding:7px 14px;border:0.5px solid var(--border);border-radius:8px;font-size:12px;color:var(--text-2);cursor:pointer;background:transparent;font-family:var(--font);">Annulla</button>
      <button onclick="saveBrief()" style="padding:7px 16px;border:0.5px solid var(--green);border-radius:8px;font-size:12px;font-weight:600;color:#003;cursor:pointer;background:var(--green);font-family:var(--font);">Salva</button>
    </div>`;
  box.appendChild(footerEl);

  overlay.appendChild(box);
  document.body.appendChild(overlay);
  document.getElementById('br-nome')?.focus();
}

function saveBrief(){
  const g = id => (document.getElementById(id)?.value||'').trim();
  const nome = g('br-nome');
  if(!nome){ showToast('Inserisci un nome per il brief','warn'); return; }

  const tipo = document.getElementById('br-tipo')?.value || _briefEditTipo;
  const sezioni = BRIEF_SEZIONI[tipo] || BRIEF_SEZIONI.grafico;
  const items = currentBriefs();
  const entry = {
    id: _briefEditIdx!==null ? items[_briefEditIdx].id : genId('br_'),
    tipo,
    nome,
    ...Object.fromEntries(sezioni.map(s => [s.key, g('br-'+s.key)])),
    note: g('br-note'),
    updatedAt: new Date().toISOString(),
  };

  if(_briefEditIdx!==null){
    items[_briefEditIdx] = {...items[_briefEditIdx], ...entry};
  } else {
    entry.createdAt = entry.updatedAt;
    items.push(entry);
  }
  ...
  document.getElementById('brief-detail')?.remove();
  renderBriefTab();
  showToast(_briefEditIdx!==null?'✓ Brief aggiornato':'✓ Brief creato');
}

function deleteBrief(idx){
  showConfirm({
    title:'Elimina brief',
    body:'Eliminare questo brief? I dati andranno persi.',
    okLabel:'Elimina', type:'danger',
    onOk:()=>{
      const items = currentBriefs();
      items.splice(idx,1);
...
/* ══════════════════════════════════════════════════════════
   ...
   Stessa struttura di IdeologyBrand per compatibilità futura.
══════════════════════════════════════════════════════════ */
  ...
  _bkModal('Colore brand', `
    ...
    () => {
      ...
      updateBrandKit(bk => {
        ...
        const entry = {id: idx!==null?colors[idx].id:genId('c_'), name, hex, role, usage};
  ...
  bk.fonts.forEach((f, i) => {
    ...
    card.innerHTML = `
      <div style="width:56px;height:56px;border-radius:8px;background:var(--surface-lt);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
        <span style="font-size:28px;font-weight:${f.weight||700};font-family:'${esc(f.name)}',sans-serif;color:var(--text-1);">Aa</span>
      </div>
      <div style="flex:1;min-width:0;">
  ...
  _bkModal('Font brand', `
    ...
    () => {
      ...
      updateBrandKit(bk => {
        ...
        const entry = {id:idx!==null?fonts[idx].id:genId('f_'), name, role, weight, url};
    ...
    async () => {
      ...
      if(file){
        ...
        try{
          ...
          const fd = new FormData();
          fd.append('file',file);
          fd.append('path',`/IdeologyStudio/${cl?.name||'Condiviso'}/BrandKit/Loghi/${file.name}`);
          const res = await fetch('/api/dropbox-upload',{method:'POST',headers:{'x-ideology-key':''},credentials:'include',body:fd});
      ...
      updateBrandKit(bk=>{
        ...
        const entry={id:idx!==null?logos[idx].id:genId('l_'),name,variant,format:ext,url:_logoUrl,note};
    ...
    async () => {
      ...
      if(file){
        ...
        try{
          ...
          const fd = new FormData();
          fd.append('file',file);
          fd.append('path',`/IdeologyStudio/${cl?.name||'Condiviso'}/BrandKit/Assets/${file.name}`);
          const res = await fetch('/api/dropbox-upload',{method:'POST',headers:{'x-ideology-key':''},credentials:'include',body:fd});
          if(!res.ok) throw new Error('Upload '+res.status);
          const d = await res.json();
      ...
      updateBrandKit(bk=>{
        ...
        const entry={id:idx!==null?assets[idx].id:genId('a_'),name,categoria:cat,format:ext,url:_assetUrl,data,note};
...
/* ══════════════════════════════════════════════════════════
   REPORT MENSILE — Ideology Studio
...
/* ══════════════════════════════════════════════════════════
   QBR — Quarterly Business Review — Ideology Studio
...
/* ══════════════════════════════════════════════════════════
   ...
   ⚠️ LEGACY — trovato in una senior review (dev/architetto/UX), verificato
   nel codice di Strategia: strategySave() (src/pages/strategia/index.html)
   scrive SEMPRE clients[idx].audit/.pdm/.pdc/.strategy (annidato
   sull'oggetto cliente, nello stesso blob condiviso) — non aggiorna MAI
   queste mappe auditData/pdmData/pdcData/stratBuildData qui sotto. Da
   quando l'editing di Audit/PDM/PDC/Strategy Builder è stato tolto da
   Content (Audit/Piano Marketing/Piano Comunicazione/Strategy Builder
   restano solo in Strategia), nessuna funzione reale scrive più QUI
   dentro — solo le vecchie renderAuditTab/renderPdmTab/renderPdcTab/
   renderStratBuildTab, ormai scollegate dal menu e irraggiungibili
   dall'interfaccia. Content continua comunque a leggerle al
   caricamento e a includerle nel proprio salvataggio (CLOUD.snapshot)
   — NON sono state rimosse: /api/project fa un replace completo del
   blob condiviso (upsert, non un merge), quindi togliere questi campi
   dal payload di salvataggio li cancellerebbe per sempre dal blob al
   primo salvataggio successivo, per un beneficio puramente estetico.
   Restano quindi congelate al loro ultimo valore reale, mai più
   aggiornate.

   FONTE DI VERITÀ VERA per audit/PDM/PDC/Strategy Builder di un
   cliente: clients[idx].audit / .pdm / .pdc / .strategy (annidato
   sull'oggetto cliente stesso) — non le variabili qui sotto. Vedi
   renderDocumentiTab() più sotto in questo file, che legge
   correttamente da lì.
══════════════════════════════════════════════════════════ */
...
// ══════════════════════════════════════════════════════════
// DOCUMENTI — vista di sola lettura dei documenti strategici
// (Brand Audit, Piano di Marketing, Piano di Comunicazione, Strategy
// Builder, Funnel), richiesta esplicita dell'utente dopo aver tolto
// l'accesso in scrittura a questi moduli da Content (restano editabili
// solo in Strategia). Legge SEMPRE clients[idx].audit/.pdm/.pdc/
// .strategy/.funnel — i campi che Strategia scrive DAVVERO (verificato
// nel suo codice, strategySave() fa clients[idx]=c, mai un
// aggiornamento separato di auditData/pdmData/pdcData/stratBuildData).
// Quelle mappe separate, usate dalle vecchie viste di Content ora
// scollegate dal menu, non vengono più aggiornate da quando Strategia è
// diventata l'editor principale — probabile contenuto ormai vecchio,
// per questo Documenti non le usa.
//
// Niente form/textarea qui: solo testo, con un rimando esplicito a
// Strategia per chi vuole modificare. Le etichette dei campi di Audit
// sono le stesse, curate, di AUDIT_SEZIONI (già definita in questo
// file, 5 campi, basso rischio di disallineamento). Per PDM (18 campi)
// e PDC (15) non si duplica qui la lista curata di Strategia — si
// mostra la chiave del campo convertita in un'etichetta leggibile, per
// non dover tenere sincronizzate due copie della stessa lista in due
// file diversi.
function docLabelFromKey(key){
  return key.replace(/_/g,' ').replace(/\b\w/g, c => c.toUpperCase());
}

const DOC_STRATEGY_LABELS = {
  mod4a: {
    title: 'Mod.4a — Brainstorming Concept',
    fields: {
      target_primario: 'Target primario', problema_reale: 'Problema reale',
      differenziatore: "Cos'è unico e provabile", frase_guida: 'Frase guida del concept',
      mood_emotivo: 'Mood emotivo', esclusioni: 'Cosa escludiamo', note_sessione: 'Note sessione',
    },
  },
  mod5: {
    title: 'Mod.5 — Identità di marca',
    fields: {
      insight: 'Insight', problema: 'Problema', reason_why: 'Reason why', promessa: 'Promessa',
      key_message: 'Key message', archetipo: 'Archetipo', tov_parole_si: 'Tone of voice — parole sì',
      tov_parole_no: 'Tone of voice — parole no', claim: 'Claim', payoff: 'Payoff',
    },
  },
};

function docFieldRow(lbl, val){
  if(!val || !val.trim()) return '';
  return `<div style="margin-bottom:12px;">
    <div style="font-size:11px;font-weight:600;color:var(--text-3);text-transform:uppercase;letter-spacing:.04em;margin-bottom:3px;">${esc(lbl)}</div>
    <div style="font-size:13px;color:var(--text-1);white-space:pre-line;line-height:1.6;">${esc(val)}</div>
  </div>`;
}

function docSection(title, icon, rowsHtml, emptyHint){
  const hasContent = rowsHtml.some(r=>r);
  return `<div style="background:var(--surface);border:0.5px solid var(--border);border-radius:10px;padding:16px 18px;margin-bottom:14px;">
    <div style="font-size:14px;font-weight:600;color:var(--text-1);margin-bottom:${hasContent?'12':'4'}px;">${icon} ${esc(title)}</div>
    ${hasContent ? rowsHtml.join('') : `<div style="font-size:12px;color:var(--text-3);font-style:italic;">${esc(emptyHint)}</div>`}
  </div>`;
}

function renderDocumentiTab(){
  const body = document.getElementById('documenti-body');
  if(!body) return;
  const cl = globalClientIdx >= 0 ? clients[globalClientIdx] : null;
  if(!cl){ body.innerHTML='<div style="text-align:center;padding:60px 0;color:var(--text-3);font-size:13px;">Seleziona un cliente per vedere i documenti.</div>'; return; }

  const a = cl.audit || {};
  const pdm = cl.pdm || {};
  const pdc = cl.pdc || {};
  const strat = cl.strategy || {};
  const funnel = cl.funnel || {};

  let html = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
      <div>
        <h2 style="font-size:20px;font-weight:600;color:var(--text-1);margin-bottom:3px;">Documenti</h2>
        <p style="font-size:12px;color:var(--text-3);">${esc(cl.name)} — sola lettura, si modifica in Strategia</p>
      </div>
    </div>
    <div style="font-size:12px;color:var(--text-2);background:var(--surface-lt);border-radius:8px;padding:10px 14px;margin-bottom:20px;">
      Questi documenti sono di sola consultazione. Per modificarli vai su Strategia → ${esc(cl.name)}.
    </div>`;

  html += docSection('Brand Audit', '🔍',
    AUDIT_SEZIONI.map(s => docFieldRow(s.lbl, a[s.key])),
    'Non ancora compilato in Strategia.');

  html += docSection('Piano di Marketing', '📊',
    Object.keys(pdm).map(k => docFieldRow(docLabelFromKey(k), pdm[k])),
    'Non ancora compilato in Strategia.');

  html += docSection('Piano di Comunicazione', '📣',
    Object.keys(pdc).map(k => docFieldRow(docLabelFromKey(k), pdc[k])),
    'Non ancora compilato in Strategia.');

  ['mod4a','mod5'].forEach(modKey => {
    const modInfo = DOC_STRATEGY_LABELS[modKey];
    const modData = strat[modKey] || {};
    html += docSection(modInfo.title, '🧠',
      Object.entries(modInfo.fields).map(([k,lbl]) => docFieldRow(lbl, modData[k])),
      'Non ancora compilato in Strategia.');
  });

  ['tofu','mofu','bofu'].forEach(stageId => {
    const st = ADS_FUNNEL_LABELS[stageId];
    const stageData = funnel[stageId] || {};
    html += docSection(st.lbl, '🔁',
      FUNNEL_FIELDS.map(f => docFieldRow(f.lbl, stageData[f.key])),
      'Non ancora compilato in Strategia.');
  });
...
function renderBrandPortalTab(){
  ...
  if(_bpSection === 'overview'){
    ...
    content.innerHTML = `<h3 style="font-size:18px;font-weight:600;color:var(--text-1);margin-bottom:16px;">Pilastri contenuto</h3>
      <div style="color:var(--text-3);font-size:12px;">I pilastri editoriali si gestiscono nella loro scheda qui accanto.</div>
...
function renderStratOverviewTab(){
  ...
  kpiRow.style.cssText = 'display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:10px;margin-bottom:24px;max-width:300px;';
  [
    ...
    el.innerHTML = `
      ...
      <div style="height:3px;background:var(--border);border-radius:0;overflow:hidden;margin-bottom:6px;">
